  ...
  if(!output_options[[3]]){   # if 'suppress posterior plots' is NOT checked
    ...
    if(mix$n.fe == 0){ # only if there are no fixed effects, otherwise p.global is meaningless
      ...
      print(ggplot2::ggplot(df, ggplot2::aes(x=x, fill=sources, colour=sources)) +
              ggplot2::geom_density(alpha=0.6, ggplot2::aes(y=..scaled..)) +
              ggplot2::scale_fill_brewer(palette = "Paired",
                                         aesthetics = c("colour", "fill"),
                                         labels = c(
                                           expression(italic("Arripis georgianus")),
                                           expression(italic("Cnidoglanis macrocephalus")),
                                           expression(italic("Hyporhamphus melanochir")),
                                           "Macro algae",
                                           expression(italic("Pelates octolineatus")),
                                           "Plankton",
                                           expression(italic("Pomatomus saltatrix")),
                                           expression(italic("Pseudocaranx sp.")),
                                           expression(italic("Sillago schomburgkii"))
                                         )) +
              ggplot2::theme_bw(16) +
              ...
              ggplot2::theme(legend.position=c(0.99,0.99), legend.justification=c(1,1), legend.title=ggplot2::element_blank()))
    ...
    if(n.effects >= 1 & mix$n.fe != 2){
      ...
      for(f1 in 1:mix$FAC[[1]]$levels){    # formerly factor1_levels
        ...
        post_density_plot <- ggplot2::ggplot(df, ggplot2::aes(x=x, fill = sources, colour=sources)) +
        ...
        plot_rds <- file.path(paste(getwd(),"/",output_options[[5]],"_diet_p_",mix$FAC[[1]]$labels[f1],".rds",sep=""))
        saveRDS(post_density_plot, plot_rds)
        print(post_density_plot)
      ...
      if(n.re==2){
        ...
        for(f2 in 1:mix$FAC[[2]]$levels){  # formerly factor2_levels
          ...
          post_density_plot <- ggplot2::ggplot(df, ggplot2::aes(x=x, fill=sources, colour=sources)) +
                  ...
                  ggplot2::labs(title = my.title) +
                  ggplot2::theme(legend.position=c(0.99,0.99), legend.justification=c(1,1), legend.title=ggplot2::element_blank())
          plot_rds <- file.path(paste(getwd(),"/",output_options[[5]],"_diet_p_",mix$FAC[[1]]$labels[f1],"_",mix$FAC[[2]]$labels[f2],".rds",sep=""))
          saveRDS(post_density_plot, plot_rds)
          print(post_density_plot)
    ...
    if(n.re > 0 || output_options[[17]]){ # only have an SD posterior plot if we have Individual, Factor1, or Factor2 random effects)
      ...
      print(ggplot2::ggplot(df2, ggplot2::aes(x=x, fill=level, colour=level)) +
              ...
              ggplot2::theme(legend.position=c(0.99,0.99), legend.justification=c(1,1), legend.title=ggplot2::element_blank()))   # + xlim(0,2)
